...
function section_teamadmin_permissions(){
  return array(
    'edit_any_team'      => 'Allow user to edit or dismiss any team'
  );
}
  ...
  if( $obj->status == 'deleted' )
    ...
    if($f_ok_x) {
      ...
      if($f_password1 != $f_password2) {
        ...
        if($f_password1 != "") {
          $cypher = ", password='".crypt($f_password1, '$1')."'";
        } else {
          $cypher = "";
        }
      }
    ...
    if($error) {
      ...
      echo '<tr><td colspan=2>Comment:<br><textarea name=f_comment cols=50 rows=6>'.$f_comment.'</textarea></td></tr>
        <tr><td colspan=2><hr></td></tr>
        <tr><td align=center colspan=2><table><tr><td>'
        .htmlFormButton ('&nbsp;OK&nbsp;', 'f_ok_x') .'</td><td width=8></td><td>'
        .htmlURLbutton ('Cancel', 'teaminfo', "id=$id", CLRBUT)
