...
<p class="pages">Go to: {if $input.page < 2}Previous{else}<a href="playersearch.php?callsign={$input.callsign|escape:"url"}&amp;page={$input.page-1}">Previous</a>{/if}
{  section name=foo start=1 loop=$data.playersearch.pages+1 step=1}
{    if $smarty.section.foo.index == $input.page}
  {$smarty.section.foo.index}
{    else}
  <a href="playersearch.php?callsign={$input.callsign|escape:"url"}&amp;page={$smarty.section.foo.index}">{$smarty.section.foo.index}</a>
{    /if}
{  /section}
{if $input.page >= $data.playersearch.pages}Next{else}<a href="playersearch.php?callsign={$input.callsign|escape:"url"}&amp;page={$input.page+1}">Next</a></p>{/if}
{/if}
