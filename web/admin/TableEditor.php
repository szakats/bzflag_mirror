<?php
    ...
    * Copyright (c) 2007 Daniel Remenak
    *
    * All rights reserved.
    *
        ...
        */
        function TableEditor($db, $table)
        {
            ...
            */
            ...
            $this->config['title']            = 'BZFlag MOTD Editor';
        ...
        <?foreach($this->fields as $field => $f):?>
            <?if(empty($f['noDisplay'])):?>
                <th style="white-space: nowrap">
                    <a href="javascript: orderBy('<?=$field . ($this->orderby['field'] == $field && $this->orderby['direction'] == 1 ? ':desc' : '') ?>')"><?=$f['display']?></a><?if($this->orderby['field'] == $field):?><?=($this->orderby['direction'] == 1 ? '&nbsp;&and;' : '&nbsp;&or;')?><?endif?>
                </th>
            <?endif?>
        <?endforeach?>
    </tr>
        ...
        */
        function handleView($id)
        {
            ...
            */
            ...
            $row = $this->dbGetRow($s = "SELECT $fields FROM $tables WHERE $joinClause AND $filters AND {$this->pk} = $quotedId");
        ...
        function displayAddEditCopyPage($id) // $id is already SQL ready
        {
            ...
            if (isset($_GET['edit']) OR isset($_GET['copy'])) {
                ...
                */
                ...
                // Get row data
                $row = $this->dbGetRow($s = "SELECT $fields FROM $tables WHERE $joinClause AND $filters AND {$this->pk} = $id");
